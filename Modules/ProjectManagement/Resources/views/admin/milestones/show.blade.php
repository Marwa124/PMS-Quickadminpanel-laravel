        ...
        <div class="col-3">
            <div class="card">
                ...
                <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                    <a class="nav-link active" id="v-pills-details-tab" data-toggle="pill" href="#v-pills-details" role="tab" aria-controls="v-pills-details" aria-selected="true">{{ trans('cruds.milestone.title') }} Details</a>
                    <a class="nav-link" id="v-pills-tasks-tab" data-toggle="pill" href="#v-pills-tasks" role="tab" aria-controls="v-pills-tasks" aria-selected="false"> {{ trans('cruds.task.title') }}<span class="float-right">{{$milestone->tasks->count() > 0 ? $milestone->tasks->count() : ''}}</span></a>
                ...
                <div class="tab-pane fade show active" id="v-pills-details" role="tabpanel" aria-labelledby="v-pills-details-tab">
                    <div class="card">
                        <h5 class="card-header">{{ $milestone->name }}
                            @can('milestone_edit')
                                <a class="float-right small" href="{{ route('projectmanagement.admin.milestones.edit', $milestone->id) }}">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between">
                                <div class="col-sm-4 border-right ">
                                    ...
                                    <div class="pl-1 ">
                                        <div class="row"> <p class="font-bold col-md-6">{{ trans('cruds.milestone.fields.name') }} :</p> <span class="col-md-6">{{ $milestone->name }}</span> </div>
                                        <div class="row"> <p class="font-bold col-md-6">{{ trans('cruds.project.title') }} {{ trans('cruds.project.fields.name') }}  : </p><span class="col-md-6">{{ $milestone->project->name }}</span> </div>
                                        <div class="row"> <p class="font-bold col-md-6">{{ trans('cruds.milestone.fields.start_date') }} :</p> <span class="col-md-6">{{ $milestone->start_date }}</span> </div>
                                        <div class="row"> <p class="font-bold col-md-6">{{ trans('cruds.milestone.fields.end_date') }} :</p> <span class="col-md-6">{{ $milestone->end_date }}</span> </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="nav flex-row nav-pills" id="v-pills-tab" role="tablist" aria-orientation="horizontal">
                                ...
                                @can('task_create')
                                    <a class="nav-link" id="v-pills-new_task-tab" href="{{route('projectmanagement.admin.tasks.create_milestone_task',$milestone->id)}}" role="tab" aria-controls="v-pills-new_task" aria-selected="false">New {{ trans('cruds.task.title_singular') }}</a>
                                @endcan
                            </div>
                        </div>
                    </div>
