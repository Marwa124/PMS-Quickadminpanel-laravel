                        ...
                        @forelse($tasks as $key => $task)
                            <tr data-entry-id="{{ $task->id }}">
                                ...
                                <td>
                                    @if(!$trashed)
                                        ...
                                        @can('task_assign_to')
                                            ...
                                            <a class="btn btn-xs btn-success {{$task->project->department ? '' : 'disabled'}}"
                                               ...
                                               title="{{$task->project->department ? '' : 'add department to project'}}">
                                    ...
                                    @else
                                        @can('task_delete')
                                            <form action="{{ route('projectmanagement.admin.tasks.forceDestroy', $task->id) }}"
                                                  method="POST"
                                                  onsubmit="return confirm('{{ trans('global.areYouSure') }}');"
                                                  style="display: inline-block;">
                                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                            ...
                                            <form action="{{ route('projectmanagement.admin.tasks.forceDestroy', $task->id) }}"
                                                  method="POST"
                                                  onsubmit="return confirm('Sub-Tasks In This Task Will Force Delete Too ..! \n{{ trans('global.areYouSure') }}');"
                                                  style="display: inline-block;">
                                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
