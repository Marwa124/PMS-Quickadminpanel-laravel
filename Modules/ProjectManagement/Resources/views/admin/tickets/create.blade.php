...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("projectmanagement.admin.tickets.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="col-sm-6">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}" name="project_id" id="project_id">
                        ...
                        @foreach($projects as $id => $v_project)
                            @if($project)
                                <option value="{{ $id }}" {{ old('project_id',$project->id) == $id ? 'selected' : 'disabled' }}>{{ $v_project }}</option>
                            @else
