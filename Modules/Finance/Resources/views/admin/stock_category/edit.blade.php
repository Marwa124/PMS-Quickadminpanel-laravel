...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.stock_category.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("finance.admin.stock_category.update", $sub_category->id) }}">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('sub_category') ? 'is-invalid' : '' }}"
                    ...
                    @foreach($all_categories as $category)
                        <option value="{{$category->id}}" {{ $sub_category->stock_category_id == $category->id ? 'selected' : '' }}>{{$category->name}}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_sub_category') ? 'is-invalid' : '' }}" type="text"
                       name="name_sub_category" id="name_sub_category" value="{{ old('name_sub_category',  $sub_category->name) }}"
                       required>
