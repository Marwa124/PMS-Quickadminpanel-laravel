            ...
            <form method="POST" action="{{ route("finance.admin.stocks_history.update") }}" enctype="multipart/form-data">
                        ...
                        @foreach($stock_categories as $stock_category)
                            <optgroup label="{{$stock_category->name}}">
                                @foreach($stock_category->sub_categories as $id => $stock_sub_category)
                                    <option value="{{ $stock_sub_category->id }}" {{ (old('stock_sub_category_id') ? old('stock_sub_category_id') : $stock->stock_sub_category->id ?? '') == $stock_sub_category->id ? 'selected' : '' }}>{{ $stock_sub_category->name }}</option>
                                @endforeach
                            </optgroup>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('buying_date') ? 'is-invalid' : '' }}" type="date"
                           name="buying_date" id="buying_date" value="{{ old('buying_date', $stock->buying_date) }}"
                           required>
