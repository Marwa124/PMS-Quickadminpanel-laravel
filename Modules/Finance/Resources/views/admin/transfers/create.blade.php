...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.transfers.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("finance.admin.transfers.store") }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('from_account') ? 'is-invalid' : '' }}"
                            name="from_account" id="from_account_id" required>
                        @foreach($accounts as $id => $account)
                            <option
                                value="{{ $account->id }}" {{ old('from_account') == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('to_account') ? 'is-invalid' : '' }}"
                            name="to_account" id="to_account_id" required>
                        @foreach($accounts as $id => $account)
                            <option
                                value="{{ $account->id }}" {{ old('to_account') == $account->id ? 'selected' : '' }}>{{ $account->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label
                        class="required" for="payment_method_id">{{ trans('cruds.transfers.fields.payment_method') }}</label>
                    <select class="form-control select2 {{ $errors->has('payment_method_id') ? 'is-invalid' : '' }}"
                        ...
                        <option selected disabled
                            value="" selected>@lang('cruds.transfers.fields.select_payment_method')</option>
