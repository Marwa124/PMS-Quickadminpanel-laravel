...
@can('customer_group_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('materialssuppliers.admin.customer-groups.create') }}">
                        ...
                        <tr data-entry-id="{{ $customerGroup->id }}">
                            ...
                            <td>
                                @can('customer_group_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('materialssuppliers.admin.customer-groups.show', $customerGroup->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('customer_group_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('materialssuppliers.admin.customer-groups.edit', $customerGroup->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('customer_group_delete')
                                    <form action="{{ route('materialssuppliers.admin.customer-groups.destroy', $customerGroup->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('customer_group_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('materialssuppliers.admin.customer-groups.massDestroy') }}",
